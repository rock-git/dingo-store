...
TEST_F(RocksLogStorageTest, sync) {
  ...
  int64_t entry_num = 100;
  for (int64_t index = 1; index < entry_num; index += (step + 1)) {
    std::vector<IndexRangeOption> options = {
        IndexRangeOption{region_id, index, index + step, 1, dingodb::wal::LogEntryType::kEntryTypeData},
    };
...
TEST_F(RocksLogStorageTest, GetTerm) {
  ...
  for (;;) {
    ...
    int64_t term = log_storage->GetTerm(region_id, index + step);
    ASSERT_EQ(1, term);
...
TEST_F(RocksLogStorageTest, Append) {
  ...
  std::vector<IndexRangeOption> options = {
      // term 1
      IndexRangeOption{10000, 1, 10, 1, dingodb::wal::LogEntryType::kEntryTypeData},
      IndexRangeOption{10001, 1, 14, 1, dingodb::wal::LogEntryType::kEntryTypeData},
      IndexRangeOption{10002, 1, 20, 1, dingodb::wal::LogEntryType::kEntryTypeData},
      // term 2
      IndexRangeOption{10000, 11, 12, 2, dingodb::wal::LogEntryType::kEntryTypeConfiguration},
      IndexRangeOption{10001, 15, 16, 2, dingodb::wal::LogEntryType::kEntryTypeConfiguration},
      IndexRangeOption{10002, 21, 22, 2, dingodb::wal::LogEntryType::kEntryTypeConfiguration},
      // term 3
      IndexRangeOption{10000, 13, 50, 3, dingodb::wal::LogEntryType::kEntryTypeData},
      IndexRangeOption{10001, 17, 80, 3, dingodb::wal::LogEntryType::kEntryTypeData},
      IndexRangeOption{10002, 23, 90, 3, dingodb::wal::LogEntryType::kEntryTypeData},
  };
...
void* WorkFunc(void* arg) {
  ...
  int64_t term = 1;
  int64_t step = 0;
  for (int64_t index = 1; index < INT64_MAX;) {
    std::vector<IndexRangeOption> options;
    if (index % 64 == 0) {
      options = {
