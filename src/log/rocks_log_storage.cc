...
DECLARE_int32(rate_bytes_per_sec);

namespace wal {

const uint32_t kPreAllocLogEntrySize = 1024;
const uint32_t kPreAllocMutationSize = 256;

const char kLogDataPrefix = 'D';
const char kLogIndexMetaPrefix = 'M';

DEFINE_int32(sync_gap_time_us, 100, "sync gap time");

static bvar::LatencyRecorder g_append_entry_latency("dingo_rocks_raft_log_append");
...
static int ExecuteRoutine(void* meta, bthread::TaskIterator<Mutation*>& iter) {  // NOLINT
  ...
  mutations.reserve(kPreAllocMutationSize);

  std::vector<WriteOp> write_ops;
  write_ops.reserve(kPreAllocLogEntrySize);

  int64_t last_time = Helper::TimestampUs();

  auto sync_log_func = [&]() {
    if (mutations.empty()) return;
    if (write_ops.empty()) return;

    if (mutations.size() > 20 || write_ops.size() > 512) {
      LOG(INFO) << fmt::format("[raft.log] sync log, mutations size({}) write op size({}).", mutations.size(),
                               write_ops.size());
    }
  ...
  for (; iter; ++iter) {
    ...
    log_storage->AppendToWriteBatch(mutation, write_ops);

    if (Helper::TimestampUs() > last_time + FLAGS_sync_gap_time_us) {
      sync_log_func();

      last_time = Helper::TimestampUs();
    }
  }
...
bool RocksLogStorage::InitRocksDB() {
  ...
  rocksdb::Status status = rocksdb::DB::Open(db_options, path_, &db);
  if (!status.ok()) {
...
bool RocksLogStorage::CommitMutation(Mutation* mutation) {
  if (BAIDU_UNLIKELY(bthread::execution_queue_execute(queue_id_, mutation) != 0)) {
    DINGO_LOG(ERROR) << fmt::format("[raft.log] execution queue execute failed.");
    return false;
  }
...
void RocksLogStorage::AppendToWriteBatch(Mutation* mutation, std::vector<WriteOp>& write_ops) {
  switch (mutation->type) {
    case Mutation::Type::kAppendLogEntry: {
      std::vector<pb::common::KeyValue> kvs;
      kvs.reserve(mutation->log_entries.size());
    ...
    case Mutation::Type::kTruncateSuffix: {
      ...
      write_op.value_or_end_key = Codec::EncodeKey(mutation->region_id, mutation->end_index + 1);
    ...
    default:
      DINGO_LOG(FATAL) << fmt::format("[raft.log] unknown mutation type({}).", static_cast<int>(mutation->type));
      break;
  }
}
...
bool RocksLogStorage::ApplyWriteOp(const std::vector<WriteOp>& write_ops) {
  ...
  for (const auto& write_op : write_ops) {
    ...
    if (write_op.type == WriteOp::Type::kPut) {
    ...
    switch (mutation->type) {
      ...
      case Mutation::Type::kTruncatePrefix: {
        ...
        if (mutation->end_index >= first_index) {
          UpdateFirstIndex(mutation->region_id, mutation->end_index);
          if (last_index < mutation->end_index) {
            UpdateLastIndex(mutation->region_id, mutation->end_index - 1);
          }
        }
      } break;
      ...
      case Mutation::Type::kTruncateSuffix: {
        ...
        if (mutation->start_index <= last_index) {
          UpdateLastIndex(region_id, mutation->start_index);
          if (mutation->start_index <= first_index) {
            UpdateFirstIndex(region_id, mutation->start_index + 1);
          }
        }
...
bool RocksLogStorage::FlushWal() {
  auto status = db_->FlushWAL(true);
  if (!status.ok()) {
    DINGO_LOG(INFO) << fmt::format("[raft.log] flush wal fail, error: {}.", status.ToString());
    return false;
  }
...
int64_t RocksLogStorage::FirstLogIndex(int64_t region_id) {
  ...
  // DINGO_LOG(INFO) << fmt::format("[raft.log][{}] first log index({}).", region_id, first_index);
...
int64_t RocksLogStorage::LastLogIndex(int64_t region_id) {
  ...
  // DINGO_LOG(INFO) << fmt::format("[raft.log][{}] last log index({}).", region_id, last_index);
...
int64_t RocksLogStorage::GetTerm(int64_t region_id, int64_t index) {
  ...
  // DINGO_LOG(INFO) << fmt::format("[raft.log][{}] get term({}) by index({}).", region_id, term, index);
...
int RocksLogStorage::TruncateSuffix(ClientType client_type, int64_t region_id, int64_t keep_last_index) {
  ...
  mutation.client_type = client_type;
  mutation.region_id = region_id;
  mutation.start_index = keep_last_index;
  mutation.end_index = INT64_MAX;

  return CommitMutation(&mutation) ? 0 : -1;
}

void RocksLogStorage::DestroyRegionLog(int64_t region_id) {
...
braft::LogEntry* RocksLogStorageWrapper::get_entry(const int64_t index) {
  // DINGO_LOG(INFO) << fmt::format("[raft.log][{}] get entry, index({}).", region_id_, index);
...
int RocksLogStorageWrapper::append_entries(const std::vector<braft::LogEntry*>& entries, braft::IOMetric* metric) {
  ...
  g_append_entry_latency << elapsed_time;
